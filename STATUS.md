...
## 2026-08-27 - V3.7.1 首次使用体验优化（本地待发布）

- 已增加 `传统 App / 智能体 / 混合产品` 的需求判定。智能体或混合产品必须建立目标触发、主动上下文、计划、工具行动、观察修正、结果证据与人工接管的 `AI-FLOW-*` 闭环。
- 已明确表单只用于必要约束、授权或人工复核；普通表单提交不能替代智能体任务完成。验收新增任务完成率、人工介入率、正确工具行动率、主动获取上下文成功率和安全拦截。
- 智能体主任务现支持用户目标、授权事件和定时计划触发；定时/事件任务必须记录频率、时区、状态、预算、通知、暂停/取消、幂等、重试与停止条件，高风险行动不自动越权。
- 已加入“先读后问”的智能体澄清短循环：先读取已有资料和授权上下文，再展示可纠正的理解卡；每轮只问一个高影响决策并提供推荐，达到目标、边界、验收、权限和风险可执行阈值后停止澄清并进入连续执行。
- 已完成 `docs/21-首次使用体验走查.md`：智能体与传统内部工具均从一句目标开始，首次回复只给理解、基础版建议、事实/推断/风险和必要决定；PRD 与开工合并为一次版本确认，定时计划改为在确认重复价值后由 AI 主动建议。
- 已重建 `v3.7.1` 本地安装包：13 个可安装文件与源码逐项 SHA256 一致，zip SHA256 为 `3449F074D458754FA0926C7AA1E031275FB21E980B044801E9179BDA7B2158D3`，大小 `64114` 字节；`git diff --check` 通过。
- 已推送 `main` 并创建 GitHub `v3.7.1` 标签；安装包直链为 `https://github.com/xiujiayou888-xiuxiu/project-workflow/raw/refs/tags/v3.7.1/project-workflow-skill.zip`。GitHub CLI 的 Release 令牌仍失效，因此本次以标签和可直接下载的安装包发布；已发布 `v3.6.0` Release 保持不变。

## 2026-08-27 - V3.6 当前版本连续执行（已发布）

- 已将用户确认的执行方式写入根 Skill、自动调度模块和状态/执行清单模板：当前版本开工确认一次后，当前总控 Agent 与可控子 Agent 连续完成任务、测试、总装和验收，不按步骤、卡片或波次要求用户点击“下一步”。
- 再次确认仅用于高风险操作、边界或验收不清，或执行中必须由用户决定的真实问题；外部不可控 AI 的复制、切换或授权只在实际交接点确认。
- 已重建 `v3.6.0` 本地安装包：13 个可安装文件与源码逐项 SHA256 一致，zip SHA256 为 `4B335AEA2A9BFFA1336C71C4F995BF1441C432398481CB5AB98F6AE040BE4615`，大小 `55502` 字节；`git diff --check` 通过。
- 已创建 `v3.6.0` Git tag、GitHub Release 与 `project-workflow-skill.zip` 资产：`https://github.com/xiujiayou888-xiuxiu/project-workflow/releases/tag/v3.6.0`。
